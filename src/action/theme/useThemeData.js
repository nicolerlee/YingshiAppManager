...
const action = {
  getStyles() {
    return themeModule66.config.styles;
  },
  getSelectedStyle() {
    return themeModule66.config.styles[themeModule66.data.sel];
  },
  getStyleComponents() {
    const components = [];
    themeModule66.config.styles.forEach((style) => {
  ...
  downloadComponentLess(componentName, themeObj, tid) {
    ...
    console.error('to download', componentName, myTheme);
    const lessCode = this.downloadPlanePaymentLess(myTheme, tid);
    return convertRpxToPx(lessCode || '');
  },

  downloadPlanePaymentLess(themeObj, tid) {
    if (themeObj.style == 1) {
      return `.pay66v${tid} {
      ${mock66.s1}
      }
